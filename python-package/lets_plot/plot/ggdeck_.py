...
def _parse_color_rgb(bg_color):
    """
    Parse *bg_color* to an (R, G, B) tuple, or return None when unparseable.

    Resolution order:
    1. CSS hex literal (#rrggbb or #rgb), handled inline — fast, no dependencies.
    2. ``webcolors`` package (standard on Python ≥ 3.11, optional before) for
       full 148-name CSS coverage.
    3. Built-in ``_NAMED_COLORS`` dict as a dependency-free final fallback.
    """
    if bg_color is None:
        return None
    ...
    if c.startswith('#'):
        ...
        if len(h) == 6:
            try:
                ...
                return None
        return None  # Unrecognised hex length

    # --- 2. webcolors (optional) ----------------------------------------
    try:
        import webcolors
        hex_val = webcolors.name_to_hex(c)  # raises ValueError if unknown
        h = hex_val.lstrip('#')
        return int(h[0:2], 16), int(h[2:4], 16), int(h[4:6], 16)
    except Exception:
        pass

    # --- 3. Curated fallback dict ---------------------------------------
    return _NAMED_COLORS.get(c)


def _contrast_text_color(bg_color):
    """Return 'black' or 'white' for best contrast against *bg_color*."""
    rgb = _parse_color_rgb(bg_color)
    if rgb is None:
        # Unknown color or explicit None: default to mid-grey luminance → white
        r, g, b = 128, 128, 128
    else:
        r, g, b = rgb
