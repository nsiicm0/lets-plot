...
import org.jetbrains.letsPlot.core.plot.base.tooltip.layout.LayoutManager.VerticalAlignment.*
...
class LayoutManager(
    ...
    private fun calculateCornerTooltipsPosition(tooltips: List<MeasuredTooltip>): List<PositionedTooltip> {
        ...
        tooltips
            ...
            .forEach { (tooltipAnchor, cornerTooltips) ->
                val tooltipsHeight = cornerTooltips.sumOf { it.size.y } + margin * cornerTooltips.size
                val verticalTooltipRange = when (tooltipAnchor.verticalAnchor) {
                    TooltipAnchor.VerticalAnchor.TOP -> rightAligned(
                        myVerticalTooltipSpace.lowerEnd,
                        tooltipsHeight,
                        0.0
                    )

                    TooltipAnchor.VerticalAnchor.BOTTOM -> leftAligned(
                        myVerticalTooltipSpace.upperEnd,
                        tooltipsHeight,
                        0.0
                    )
                ...
                cornerTooltips.forEach { tooltip ->
                    ...
                    tooltipY += positionedTooltip.height + margin
                }
            }
        ...
        horizontalsWithOverlappedCorners
            .let { horizontalTooltips ->
                if (horizontalTooltips.sumOf(PositionedTooltip::height) < myVerticalSpace.length) {
                    HorizontalTooltipExpander(margin, myVerticalSpace)
            ...
            .forEach { tooltip: PositionedTooltip ->
                val newTooltip = if (placementList.isOverlapped(tooltip)) {
                    val newPosition = DoubleVector(
                        placementList.last().rect().right + margin,
    ...
    private fun calculateAnchorX(measuredTooltip: MeasuredTooltip, horizontalAlignment: HorizontalAlignment): Double {
        return when (horizontalAlignment) {
            ...
            HorizontalAlignment.LEFT -> myHorizontalSpace.lowerEnd + myHorizontalTooltipSpace.lowerEnd + margin
    ...
    private fun isAxisTooltip(tooltipSpec: TooltipSpec) =
        tooltipSpec.layoutHint.kind in listOf(X_AXIS_TOOLTIP, Y_AXIS_TOOLTIP)
    ...
    class PositionedTooltip {
        ...
        val hintKind get() = tooltipSpec.layoutHint.kind
